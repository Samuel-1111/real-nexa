...
export default function LandingGate(){
 ...
 useEffect(()=>{if(window.location.pathname!=="/")return;const seen=localStorage.getItem("nexa_landing_seen")==="1";if(seen){createClient().auth.getUser().then(({data})=>{if(data.user)window.location.replace("/home");else setShow(true)}).catch(()=>setShow(true));return;}setShow(true);const t=window.setTimeout(()=>setReady(true),1800);return()=>window.clearTimeout(t)},[]);
 if(!show)return null;
