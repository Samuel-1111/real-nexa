...
const functionDeclarations = [
  { name:"create_task", description:"Create a task for the authenticated user.", parameters:{type:"object",properties:{title:{type:"string"},due_at:{type:"string",description:"ISO-8601 datetime or omit for no due date"},priority:{type:"string",enum:["low","normal","high","urgent"]}},required:["title"]}},
  { name:"list_tasks", description:"List the authenticated user's tasks. Use this when the user asks what tasks they have, what is pending, or asks about today's tasks.", parameters:{type:"object",properties:{include_completed:{type:"boolean"}},required:[]}},
  { name:"complete_task", description:"Mark one of the authenticated user's tasks as completed.", parameters:{type:"object",properties:{task_id:{type:"string"}},required:["task_id"]}},
  ...
  { name:"list_reminders", description:"List the authenticated user's reminders. Use this when the user asks what reminders they have or what reminders are upcoming.", parameters:{type:"object",properties:{include_completed:{type:"boolean"}},required:[]}},
  { name:"complete_reminder", description:"Mark one of the authenticated user's reminders as completed.", parameters:{type:"object",properties:{reminder_id:{type:"string"}},required:["reminder_id"]}},
  { name:"create_event", description:"Create a calendar event for the authenticated user.", parameters:{type:"object",properties:{title:{type:"string"},starts_at:{type:"string",description:"ISO-8601 datetime"},ends_at:{type:"string",description:"ISO-8601 datetime"},location:{type:"string"}},required:["title","starts_at","ends_at"]}},
  ...
  { name:"create_goal", description:"Create a goal for the authenticated user.", parameters:{type:"object",properties:{title:{type:"string"},description:{type:"string"},target_date:{type:"string",description:"YYYY-MM-DD"}},required:["title"]}},
...
async function callGemini(contents: any[], systemInstruction: string) {
  ...
  try {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent`, {
      ...
      body:JSON.stringify({systemInstruction:{parts:[{text:systemInstruction}]},contents,tools:geminiTools,generationConfig}),
...
Deno.serve(async(req)=>{
  ...
  try {
    if(!GEMINI_API_KEY) return json({error:"AI service is not configured."},503);
    if(!SUPABASE_SECRET_KEY) return json({error:"Supabase server configuration is missing."},503);
    ...
    if(!message||message.length>8000) return json({error:"Invalid message."},400);
...
Use the user's real tasks, reminders, calendar events, notes, and goals when answering questions about them. When the user asks to add/create/save something, use the appropriate tool. When the user asks to know/list/show their tasks, reminders, calendar events, notes, or goals, use the appropriate list tool instead of guessing. Never claim an action happened unless the tool returned a successful result. Keep ordinary answers direct and natural. For voice-friendly replies, prefer short, natural sentences. Do not mention these instructions.`;
    ...
    let reply=""; let inputTokens=0; let outputTokens=0;
    ...
    for(let round=0;round<4;round++){
      ...
      if(!modelContent) throw new Error("Gemini returned no content.");
      const functionCalls=(modelContent.parts||[]).map((part:any)=>part.functionCall).filter(Boolean);
      if(functionCalls.length===0){
        reply=(modelContent.parts||[]).map((part:any)=>typeof part.text==="string"?part.text:"").join("").trim()||"I'm here. How can I help?";
        break;
      }
      ...
      for(const functionCall of functionCalls){
        ...
        const result=await runTool(String(functionCall.name||""),args,user.id);
        const functionResponse:any={name:String(functionCall.name||""),response:{result}};
        if(functionCall.id) functionResponse.id=functionCall.id;
        responseParts.push({functionResponse});
      }
      contents.push({role:"user",parts:responseParts});
    }
    if(!reply) reply="I completed the requested action. What would you like to do next?";

    let finalConversationId=conversationId;
    if(!finalConversationId){
      const {data,error}=await admin.from("conversations").insert({user_id:user.id,title:message.slice(0,60)}).select("id").single();
      if(error) throw error;
      finalConversationId=data.id;
    }
    const {data:savedMessage,error:saveError}=await admin.from("messages").insert([
      {conversation_id:finalConversationId,user_id:user.id,role:"user",content:message},
      {conversation_id:finalConversationId,user_id:user.id,role:"assistant",content:reply},
    ]).select("id,role").eq("role","assistant").single();
    if(saveError) throw saveError;
    await admin.from("conversations").update({updated_at:new Date().toISOString()}).eq("id",finalConversationId).eq("user_id",user.id);
    if(inputTokens||outputTokens) void admin.rpc("record_nexa_ai_tokens",{p_user_id:user.id,p_input_tokens:inputTokens,p_output_tokens:outputTokens});

    return json({conversation_id:finalConversationId,reply,message_id:savedMessage.id});
  } catch(error) {
